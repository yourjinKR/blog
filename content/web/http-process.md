...
이 요청은 프로토콜 스택이라는 OS에 내장된 네트워크 제어용 소프트웨어에 의해 [[packet|패킷]]에 담기고 패킷에 제어정보를 덧붙여 LAN 어댑터에 전송하고, LAN 어댑터는 이를 전기신호로 변환시켜 송출합니다.
...
핵심부를 통과한 패킷은 목적지의 LAN에 도착하고, 방화벽이 패킷을 검사한 후 [[http-cache|캐시]] 서버로 보내어 웹 서버에 갈 필요가 있는지 검사합니다.
...
https://www.youtube.com/watch?v=FwH2S_oNRk8&t=1s