...
논리 주소를 이용하여 목적지를 식별하며, 목적지까지의 경로를 결정하고 패킷을 다음 네트워크로 전달한다.  
...
Network Layer의 Packet을 Frame으로 캡슐화하며, [[MAC]] Address를 이용해 목적지를 식별한다. 또한 전송 매체 접근 제어와 오류 검출 등의 기능을 수행한다. 대표적인 장비는 Switch이며, 대표적인 기술로 Ethernet과 Wi-Fi가 있다.
...
각 계층 별로 캡슐화와 디캡슐화 과정을 통해 데이터가 전송된다.  

| 계층           | 송신 측 (캡슐화)                  | 수신 측 (디캡슐화)          |
| ------------ | --------------------------- | -------------------- |
